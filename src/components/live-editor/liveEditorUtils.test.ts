...
describe('parseCheckboxLine', () => {
  test('parses an unchecked checkbox line', () => {
  ...
  test('parses a checked checkbox line', () => {
  ...
  test('parses an indented nested checkbox line', () => {
  ...
  test('returns null for empty-titled checkbox lines', () => {
...
describe('parseAllCheckboxes', () => {
  test('collects checkboxes in document order with correct indices', () => {
  ...
  test('collects indented nested checkboxes in document order', () => {
  ...
  test('returns an empty array when there are no checkbox lines', () => {
    expect(parseAllCheckboxes('hello\nworld')).toEqual([])
  })
})

describe('setCheckboxStateAtLine', () => {
  test('flips a single line from unchecked to checked', () => {
    const before = '- [ ] todo\nfoo'
    expect(setCheckboxStateAtLine(before, 0, true)).toBe('- [x] todo\nfoo')
  })

  test('flips a single line from checked to unchecked', () => {
    const before = 'foo\n- [x] done'
    expect(setCheckboxStateAtLine(before, 1, false)).toBe('foo\n- [ ] done')
  })

  test('preserves the rest of the document verbatim', () => {
  ...
  test('preserves indentation when checking and unchecking a nested checkbox line', () => {
  ...
  test('returns the original text for non-checkbox lines', () => {
    const before = 'plain text\n- [ ] a'
    expect(setCheckboxStateAtLine(before, 0, true)).toBe(before)
  })

  test('returns the original text for out-of-range indices', () => {
    const before = '- [ ] a'
...
describe('replaceLineAtIndex', () => {
  test('restores a promoted checkbox line back to its original plain prose', () => {
  ...
  test('replaces only the target line and leaves the rest verbatim', () => {
    // Arrange
...
describe('removeLineAtIndex', () => {
  test('drops a finished checkbox line so clear-on-complete leaves the rest verbatim', () => {
  ...
  test('removes only the target line and joins the surrounding lines', () => {
    // Arrange
...
describe('insertLineAtIndex', () => {
  test('re-inserts an undone line back at its original middle position', () => {
  ...
  test('restores the only line without leaving a trailing blank line', () => {
  ...
  test('round-trips a remove then insert back to the original text', () => {
    // Arrange
    const original = '- [ ] one\n- [ ] two\n- [ ] three'
  ...
  test('drops the trailing blank line when the sole content line is cleared then restored (known asymmetry)', () => {
  ...
  test('clamps an out-of-range index to the document end', () => {
    // Arrange
    const cleared = 'a'
  ...
  test('inserts at the document head for a negative index', () => {
    // Arrange
    const cleared = 'b\nc'
...
describe('lineStartOffset', () => {
  test('returns the caret offset at the start of a middle line', () => {
  ...
  test('returns the caret offset at the start of the third line', () => {
  ...
  test('drops the caret at the document end when the index is past the last line', () => {
  ...
  test('returns 0 for the first line', () => {
    // Arrange
    const text = 'first\nsecond'
...
describe('markPlainLineCompleted', () => {
  test('wraps a plain prose line as a checked checkbox so it can be completed', () => {
    // Arrange
    const text = 'buy milk'
  ...
  test('wraps only the caret line and leaves the rest of the document verbatim', () => {
    // Arrange
  ...
  test('trims surrounding whitespace into the title and the rewritten line', () => {
    // Arrange
  ...
  test('returns null for an already well-formed checkbox line (toggle path owns it)', () => {
    // Arrange + Act + Assert
    expect(markPlainLineCompleted('- [ ] todo', 0)).toBeNull()
    expect(markPlainLineCompleted('- [x] done', 0)).toBeNull()
  })

  test('returns null for an empty checkbox skeleton so no junk title is logged', () => {
    // Arrange + Act + Assert
...
describe('normalizeCompletedTitle', () => {
  ...
  test('returns the trimmed value when within the schema limit', () => {
  ...
  test('caps the title at COMPLETED_TITLE_MAX_LENGTH', () => {
