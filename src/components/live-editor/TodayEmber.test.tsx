...
describe('TodayEmber copy — one honest line per state', () => {
  test('shows only the resolving word before the source can answer, never a made-up 0', () => {
    // Arrange / Act
  ...
  test("says the keeps cannot be reached instead of showing 0 when the account's fetch failed", () => {
    // Arrange / Act
  ...
  test('stays unlit at zero with the gentle starting line', () => {
    // Arrange / Act
  ...
  test('lights for one keep and counts in the singular', () => {
    // Arrange / Act
  ...
  test('counts in the plural and keeps the numeral in tabular figures', () => {
    // Arrange / Act
  ...
  test('compact (Electron panel) keeps the headline only', () => {
    // Arrange / Act
...
describe('TodayEmber motion — sweep on a keep, snap on an undo', () => {
  test('lights with a fresh sweep on the first keep and re-sweeps on the next one over a settled lit layer', () => {
  ...
  test('snaps back to unlit on undo with no reverse animation', () => {
  ...
  test('does not sweep when it mounts already lit (a returning visitor)', () => {
    // Arrange / Act
  ...
  test('does not sweep when the count merely resolves (undefined → 3)', () => {
    // Arrange
...
describe('TodayEmber accessibility', () => {
  test('is a polite live region whose cell is decorative', () => {
    // Arrange / Act
    render(<TodayEmber count={1} />)
