...
describe('useCompletionWriter — where a keep lands', () => {
  test('signed in, records the keep on the account and moves the ember up before any refetch', async () => {
    // Arrange
  ...
  test('leaves the ember untouched when the account write fails', async () => {
    // Arrange
  ...
  test('undo of an account keep deletes the row and moves the ember back down', async () => {
  ...
  test('undo just after local midnight takes the keep off yesterday, not off today', async () => {
  ...
  test('leaves the ember alone when undoing a keep it never saw created', async () => {
  ...
  test('never invents a cached total when nothing was fetched yet', async () => {
    // Arrange
    createCompletedFn.mockResolvedValue({ id: 42 })
    const { writer, queryClient } = renderWriter(null)
  ...
  test('signed out, keeps the line on this device and never calls the server', async () => {
    // Arrange
  ...
  test('undo of a device-local keep removes it locally even after signing in', async () => {
