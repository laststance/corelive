...
describe('useTodayKeeps — signed out, the device store', () => {
  test("counts only today's device keeps and moves the moment a line is kept", () => {
  ...
  test('hears another tab keep a line (storage event) so both embers agree', () => {
    // Arrange
    const { result } = renderTodayKeeps()
  ...
  test("leaves yesterday's keeps behind once the local day rolls over", () => {
...
describe('useTodayKeeps — signed in, the account', () => {
  test("adds the account's total to keeps made on this device before sign-in that have not merged yet", async () => {
  ...
  test('flags the account query as never-persisted, so a total from an older day cannot be replayed as today', async () => {
    // Arrange
    clerkUserRef.current = { isLoaded: true, isSignedIn: true }
    heatmapQueryFn.mockResolvedValue({
      total: 1,
  ...
  test("drops yesterday's total when the local day rolls over under a mounted editor, instead of counting it as today", async () => {
  ...
  test("shows the resolving word on the very frame the day flips, never yesterday's number for a beat", async () => {
  ...
  test('reports null — not 0 — when the fetch fails with nothing cached', async () => {
    // Arrange
    clerkUserRef.current = { isLoaded: true, isSignedIn: true }
  ...
  test('stays undefined while Clerk has not loaded, so nothing is claimed too early', () => {
    // Arrange
  ...
  test('never fetches the account total while signed out', () => {
    // Arrange / Act
    renderTodayKeeps()
