...
export const handlers = [
  ...
  http.get('https://accounts.google.com/v3/signin/identifier', ({ request }) => {
    ...
    if (redirectUri) {
      ...
      console.log('[MSW] Redirecting to Clerk callback:', callbackUrl.toString())
  ...
  http.get('https://accounts.google.com/*', ({ request }) => {
    ...
    if (redirectUri && clientId) {
      ...
      console.log('[MSW] Redirecting OAuth request to Clerk callback:', callbackUrl.toString())
  ...
  http.post('https://*.clerk.accounts.dev/v1/client/sign_ins/:signInId/authenticator/oauth_google', ({ params, request }) => {
  ...
  http.post('https://*.clerk.accounts.dev/v1/client/sign_ins', async ({ request }) => {
    ...
    if (body.includes('oauth_google')) {
      ...
      console.log('[MSW] Simulating successful Google OAuth, redirecting to /home')
  ...
  http.post('https://api.clerk.com/v1/oauth/authorize', ({ request }) => {
    console.log('[MSW] Intercepted Clerk OAuth authorize request')
    return HttpResponse.json({
      auth_session_id: 'mock_session_123',
      redirect_url: 'https://accounts.google.com/o/oauth2/v2/auth?client_id=mock&redirect_uri=http://localhost:3000/login&state=mock_state',
    })
  }),
  ...
  http.get('https://clerk.shared.lcl.dev/v1/oauth_callback', ({ request }) => {
    ...
    if (code === 'mock_auth_code_123') {
      console.log('[MSW] Processing mock OAuth callback, setting session cookies and redirecting to /home')
      
      // Create a mock Clerk session token (JWT-like structure)
      const mockSessionToken = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJjbGVyay5sb2NhbCIsInN1YiI6InVzZXJfbW9ja18xMjMiLCJleHAiOjE3NTQ3Nzk3MTIsImlhdCI6MTc1NDY5MzMxMiwic2lkIjoic2Vzc19tb2NrXzEyMyJ9.mock_signature'
