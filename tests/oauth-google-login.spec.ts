...
test.describe('Google OAuth Login with MSW', () => {
  test('should verify MSW is initialized', async ({ page }) => {
    ...
    expect(mswLogs.some((l) => l.includes('Mock Service Worker enabled'))).toBe(true)
  })
  ...
  test('should display Google OAuth button on login page', async ({ page }) => {
    ...
    const googleButton = await page.locator('button:has-text("Google"), [aria-label*="Google"], img[alt*="Google"]').first()
    ...
    const clerkSignIn = await page.locator('[data-clerk-sign-in], .cl-rootBox').first()
    const hasClerkSignIn = await clerkSignIn.isVisible().catch(() => false)
    
    console.log('Clerk SignIn component present:', hasClerkSignIn)
    expect(hasClerkSignIn).toBe(true)
  })

  test('should intercept OAuth API calls with MSW', async ({ page }) => {
    const mswLogs: string[] = []
    page.on('console', (msg) => {
      const text = msg.text()
      if (text.includes('MSW') || text.includes('Intercepted')) mswLogs.push(text)
    })
    await page.goto('/login', { waitUntil: 'domcontentloaded', timeout: 10000 })
    await page.waitForTimeout(1000)
    expect(mswLogs.some((l) => l.includes('Mocking enabled'))).toBe(true)
  })
  ...
  test('should handle OAuth callback with mock code', async ({ page }) => {
    ...
    await page.goto(mockCallbackUrl, { waitUntil: 'domcontentloaded', timeout: 10000 })
    await page.waitForTimeout(3000)
    ...
    // Take a screenshot for debugging
    await page.screenshot({ path: 'tests/screenshots/oauth-callback.png', fullPage: true })
    
    // Check if there are any error messages
    const errorMessage = await page.locator('.cl-formFieldError, .cl-formButtonPrimary__error, [role="alert"]')
      .textContent()
      .catch(() => null)
