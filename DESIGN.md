...
9. **Theme toggle = crossfade.** Switching theme crossfades rather than hard-cutting (Motion table, "Theme toggle"). As built this is a **body-only** background/text fade (200ms `ease-out`, gated behind `motion-safe:` — `prefers-reduced-motion` users get an instant change). _Open for design-review (#5): whether to broaden the fade to cards/sidebar or keep it body-only._
...
> **The heatmap's temperature = pride. This holds on every theme, forever — it is a CI-enforced invariant, not a guideline.**

Adding a colored family must never let the heatmap drift back toward "GitHub-green work-grid" or a flat lightness ramp. For **every** registered theme, `src/lib/themes/generate-theme-css.test.ts` asserts the five heatmap stops (`--hm-0` rest → `--hm-4` apex):

1. **Lightness moves monotonically** — light mode rests palest and deepens to the apex; dark mode rests darkest and brightens to the apex.
2. **Chroma intensifies toward the apex** — the fullest day is the most saturated cell, in both modes.
3. **The hottest cell lands in the shared warm band** — `--hm-4` hue ∈ **[20, 70]** regardless of where the family rests. Grove may rest green and Iris may rest violet, but the apex always blooms warm. The rest hue is the family's; the payoff hue is the project's.

Per-theme WCAG AA gates ride alongside (`fg/bg ≥ 7`, `muted-fg/card ≥ 4.5`, `primary-fg/primary ≥ 4.5`). A new family that breaks monotonicity, cools its apex, or fails AA **fails the build** — the north star cannot regress by accident.
...
| 2026-06-10 | Heatmap "temperature = pride" locked as a CI invariant across ALL themes (monotonic lightness, rising chroma, apex hue ∈ [20, 70])                                                                                            | The north-star signal must survive every future family — a green Grove still blooms warm at the apex; the build fails if a new theme cools its apex, flattens the ramp, or drops AA                                                                                                                                                          |
...
| 2026-06-11 | Theme toggle is a body-only crossfade (bg + text, 200ms ease-out, `motion-safe`); Electron native `appearance.theme` / `accentColor` removed                                                                                  | Minimal, reduced-motion-safe transition (broaden-to-cards vs keep-body-only is an open design-review call); the native theme copy had zero readers and could only drift from the real web-persisted value                                                                                                                                    |
