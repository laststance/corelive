...
corelive ships an **opinionated concept** — the North Star and Warm Cathedral. That concept owns every **default and preset**. It is not, however, a cage for the user.

**The rule:** an option that falls _outside_ the concept is allowed **only after** the on-concept choice is fully built and set as the default.

- The **default is always concept-faithful** — a fresh install embodies the concept with zero configuration.
- On-concept **presets stand on their own** — someone who never opens settings still gets the full intended experience.
- Out-of-concept options are **opt-in, never forced, never the default**, and sit one layer deeper (settings), not in the user's face.

This holds for **design, features, and UX alike** — themes, sound, motion, copy, all of it. The concept owns the default; the user owns the deviation.

**Never deviable: the North Star itself** — self-affirmation over judgment. You may offer an off-concept _aesthetic_ (a louder theme, an optional sound); you may **not** offer one that grades, shames, or guilts (no opt-in streak-shame, no KPI-percentage mode). Style is negotiable; the emotional contract is not.

> **Instances.** **Sound palette** — default is silence (on-concept); the soft cues are the opt-in layer, off per moment until chosen. **Theme system** — Warm Cathedral is the default; the colored families are opt-in deviations. Next customization? Same shape: ship the concept default, make it sufficient, then let the option ride on top.
